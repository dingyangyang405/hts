...
class TicketController extends BaseController
{
    ...
    { 
        ...
        $categorys = $this->getCategoryService()->findCategorys();
    ...
    public function reminderAction(Request $request, $id)
    {
        ...
        if (!empty($ticket['operatorId'])) {
            $this->getNotificationService()->sendNotification($ticket['userId'], $ticket['operatorId'], $message);
        }       
        return new JsonResponse(array('success' => 'true'));
    }
    ...
    public function closeAction(Request $request, $id)
    {
        ...
        if (!empty($closeTicket['operatorId'])) {
            $this->getNotificationService()->sendNotification($closeTicket['userId'], $closeTicket['operatorId'], $message);
        }
    ...
    public function replyAction(Request $request, $id)
    {
        if ($request->getMethod() == 'POST') {
            ...
            $this->getNotificationService()->sendNotification($replyTicket[0]['operatorId'], $replyTicket[0]['userId'], $message);
            }        
            return new JsonResponse(array('success' => 'true'));
        }
    }
